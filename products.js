...
// Shopify applies it: each bundle has a discount code (created in Shopify
// admin as "Amount off products", pct% off exactly these products, minimum
// quantity = number of products in the bundle). Adding a bundle puts its
// code on the Shopify cart automatically; checkout carries it; Shopify
// decides what is actually charged. Never show a bundle price here that has
// no matching code in Shopify.
//
...
// ---- bundle discount codes on the cart ----
// { bundleId: { applicable: true | false | null } } — which bundles the
// visitor added, and what Shopify last said about the code. Kept per device
// like the cart itself.
var BUNDLE_CODES_KEY = 'pawhaul_bundle_codes';
var bundleCodeState = (function () {
  try { return JSON.parse(localStorage.getItem(BUNDLE_CODES_KEY) || '{}') || {}; } catch (e) { return {}; }
})();
function saveBundleCodeState() {
  try { localStorage.setItem(BUNDLE_CODES_KEY, JSON.stringify(bundleCodeState)); } catch (e) {}
}

// A bundle's code only belongs on the cart while every product in the
// bundle is still in it — remove one and the discount (and the claim) goes.
...
function activeBundles() {
  return BUNDLES.filter(function (b) { return bundleCodeState[b.id] && bundleInCart(b); });
}
function activeBundleCodes() {
  return activeBundles().map(function (b) { return b.code; });
}

// Shopify's verdict on each code. applicable:false after a whole bundle was
// added means the code is missing or misconfigured in Shopify admin — say so
// instead of implying a discount the checkout will not give.
function noteBundleCodes(discountCodes, justAddedId) {
  (discountCodes || []).forEach(function (dc) {
    var b = BUNDLES.find(function (x) { return x.code.toUpperCase() === String(dc.code).toUpperCase(); });
    if (b && bundleCodeState[b.id]) bundleCodeState[b.id].applicable = !!dc.applicable;
  });
  saveBundleCodeState();
  if (justAddedId && bundleCodeState[justAddedId] && bundleCodeState[justAddedId].applicable === false) {
    showToast('The bundle discount could not be applied right now, so these items are at their regular price.', 6000);
  }
  var cartPage = document.getElementById('page-cart');
  if (cartPage && cartPage.classList.contains('active')) renderCart();
}

function syncBundleCodes(justAddedId) {
  return queueCartSync(async function () {
    var cartId = getStoredCartId();
    if (!cartId) return;
    var r = await cartApi({ action: 'discountCodes', cartId: cartId, discountCodes: activeBundleCodes() });
    if (r && r.ok) noteBundleCodes(r.discountCodes, justAddedId);
  });
}
...
function addBundleToCart(bundleId) {
  ...
  bundleCodeState[b.id] = { applicable: null };
  saveBundleCodeState();
  // Queued behind the adds above, so the Shopify cart exists by the time
  // the code is attached.
  syncBundleCodes(b.id);
  showToast(b.name + ' added to cart! ' + b.pct + '% off is applied at checkout.');
}

// Estimated saving for the cart page, mirroring Shopify: each active code
// takes pct% off every line of its products; where two active bundles share
// a product, the bigger percentage wins (codes are set not to combine).
function bundleSavingsForCart() {
  var bundles = activeBundles().filter(function (b) { return bundleCodeState[b.id].applicable !== false; });
  ...
  cart.forEach(function (item) {
    ...
    bundles.forEach(function (b) {
      if (bundleProducts(b).some(function (p) { return p.id === item.id; })) best = Math.max(best, b.pct);
    });
    if (best) amount += pctOff(item.price * item.qty, best);
  });
  return { amount: amount, bundles: bundles };
}
...
function removeFromCart(idx) {
  ...
  // A bundle broken by this removal loses its code (and its cart-page line).
  BUNDLES.forEach(function (b) { if (bundleCodeState[b.id] && !bundleInCart(b)) delete bundleCodeState[b.id]; });
  saveBundleCodeState();
  syncBundleCodes();
  updateCartCount();
  renderCart();
  ...
  container.innerHTML = `
    <div class="cart-layout">
      ...
      <div class="cart-summary">
        ...
        ${activeBundleCodes().length ? '' : shopPayBlockHtml()}
...
async function checkout() {
  ...
  try {
    var cartId = getStoredCartId();
    var data = null;
    // Bundle discount codes (task 112) ride along on whichever cart checkout
    // uses: set on a reused cart if its list differs, passed to a new one.
    var codes = activeBundleCodes();
    var codeKey = function (list) { return list.map(function (c) { return String(c).toUpperCase(); }).sort().join(','); };
    if (cartId) {
      var existing = await cartApi({ action: 'get', cartId: cartId });
      if (existing && existing.ok && cartMatchesLines(existing.lines, lines)) {
        data = existing;
        var have = (existing.discountCodes || []).map(function (dc) { return dc.code; });
        if (codeKey(have) !== codeKey(codes)) {
          var updated = await cartApi({ action: 'discountCodes', cartId: cartId, discountCodes: codes });
          if (updated && updated.ok) data = updated;
        }
      }
    }
    if (!data) {
      data = await cartApi({ action: 'create', lines: lines, discountCodes: codes });
